...
parameter ={
            'learning_rate' : 0.03, #일반적으로 가장 성능에 영향을 많이 끼침. 경사하강법에서 얼만큼씩 하강할것이냐를 뜻함. 웨이트를 찾을때 적절한 러닝레이트 필요
            'depth':6,
            'iterations':1000,
            'l2_leaf_reg' : 3,
            'random_seed' : 4321
...
#3 컴파일 훈련
model.set_params(**parameter, eval_metric = 'MAE', early_stopping_rounds = 100) # model.compile이라고 생각하면 된다
