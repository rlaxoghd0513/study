...
x_train,x_test, y_train, y_test = train_test_split(x,y, random_state = 333, shuffle=True, train_size = 0.9)

parameter ={
            'learning_rate' : 0.04, #일반적으로 가장 성능에 영향을 많이 끼침. 경사하강법에서 얼만큼씩 하강할것이냐를 뜻함. 웨이트를 찾을때 적절한 러닝레이트 필요
            'depth':8,
            'iterations':10000,
            'random_seed': 333,
            'l2_leaf_reg' : 4
...
#3 컴파일 훈련
model.set_params(**parameter, eval_metric = 'MAE', early_stopping_rounds = 333) # model.compile이라고 생각하면 된다
...
x_submit = test_input_dataset[test_input_dataset.isnull().any(axis=1)]
