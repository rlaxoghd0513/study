...
labels = ["Original", "Decoded_01", "Decoded_08", "Decoded_32", "Decoded_64", "Decoded_154", "Decoded_331", "Decoded_486", "Decoded_713"]
...
for row_num, row in enumerate(axes):
    for col_num, ax in enumerate(row):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_title(labels[row_num])  # 라벨추가

plt.tight_layout() # 서브플롯들 사이의 간격과 레이아웃이 최적화되어 플롯이 더 잘 보이도록 만들어집니다.
plt.show()