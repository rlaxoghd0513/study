...
model.add(Conv2D(128, 3, activation = 'relu', padding = 'same', kernel_regularizer = L1(0.02)))
model.add(BatchNormalization())
model.add(MaxPooling2D())
model.add(Conv2D(32, 3, activation = 'relu', padding = 'same', kernel_regularizer = L2(0.01)))
model.add(BatchNormalization())
model.add(MaxPooling2D())
model.add(Flatten())
