...
# 모델이 예측한 값
predicted_values = multi_output_model.predict(x_test)

better_counter = 0  # 'better'의 개수를 세기 위한 카운터
rows_with_3_better = []  # 'better'가 3개 이상인 행을 저장할 리스트
...
if better_counter > 0:
    print(f'There are {better_counter} rows with 3 or more "better"')
    print("Rows with 3 or more 'better':")
    for index, row in rows_with_3_better:
        print(f"Row index: {index}, Predicted values: {row}")
else:
