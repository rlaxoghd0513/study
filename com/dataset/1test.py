...
# 조건에 맞는 데이터 찾기
condition = (
    (predicted_values[:, 0] >= most_common_values['A']) #&
    # (predicted_values[:, 1] >= most_common_values['B']) &
    # (predicted_values[:, 2] >= most_common_values['C']) &
    # (predicted_values[:, 3] <= most_common_values['D']) &
    # (predicted_values[:, 4] <= most_common_values['E'])
)

# 조건을 만족하는 데이터 행 출력
selected_rows = x_test[condition]
print(selected_rows)
