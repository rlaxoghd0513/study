...
x_data=train_set.drop(['Outcome'], axis=1)
y_data=train_set['Outcome']
print(x_data.shape)#(652, 8)
print(y_data.shape)#(652,)
...
x = tf.compat.v1.placeholder(tf.float32, shape =[None,8]) #행의 갯수는 변경될수도 있다 그래서 None으로 명시 
...
x_test = tf.compat.v1.placeholder(tf.float32, shape=[None,8])
