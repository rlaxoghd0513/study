...
model.add(Conv2D(7, (2,2),               #이미지는 행렬rgb(채널)장수 4차원
                 input_shape = (8,8,1))) #가로가 8 세로가 8 1은 칼라 흑백  (2,2)는 자르는 크기 (7,7)로 바뀜  높은놈은 높아지고 낮은놈은 낮아지고 그 데이터를 7장으로 늘렸다
                                         #특성은 좁히고 데이터는 늘린다   통과했을때 (7.7.7)이 된다  #출력4차원   : (N,7,7,7) 
                                                                                                 # 인풋사이즈의 정식명칭 #(batch_size, lows, columns, channels) 칼라면 3 흑백이면 1 
                                                                                                #  channels는  레이어 한번 거치면 필터로 바뀜          
model.add(Conv2D(filters = 4,            
                 kernel_size = (3,3),      #얼마의 크기로 자를지 
                 activation='relu'))       # 4차원 안에 모든 값들은 relu를 거쳐 음수부분은 0이 된다 출력(N, 5,5,4)
                                            # 10,10을 2,2로 나누면 10-2+1 9,9        3,3로 나누면 10-3+1 8,8   
...
# input channel = c_in
# output channel = c out
# kernal = (k,k)
# k*k*cin*cout+cout
# (k*k*chanels+bias)*filters
#(2*2)*1*7+7  =35
#(3*3)*7*4+4 =256
#(2*2)*4*10+10 = 170
