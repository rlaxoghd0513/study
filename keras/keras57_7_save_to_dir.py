...
#증폭(증강) 
augment_size = 1000 #6만개인 패션엠니스트를 10만개로 증폭할거다 그래서 augment_size 4만
...
x_augmented = train_datagen.flow(
    ...
    batch_size = augment_size,
    shuffle = False,
    save_to_dir = 'd:/temp/'    #증폭되는 데이터 저장
).next()[0] # 여기가 증폭하는 부분

end_time = time.time() - start_time #time.time() 현재시간
...
x_train =np.concatenate((x_train/255.,x_augmented)) #합친다
