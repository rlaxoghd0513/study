...
y1_test, y2_train, y2_test = train_test_split(x1,y1,y2, train_size=0.7, random_state=333)  #train_test_split은 2개이상도 자를수 있다
...
merge1 = concatenate([output1], name = 'mg1') #모델들의 아웃풋들만 연결되면 됨 그게 컨캣한 모델의 인풋이 된다 **두개이상은 리스트** 컨캣한 줄은 연산량이 없다
...
hidden_output = Dense(64, name ='hidden_output')(merge3)  #얘가 이제 마지막이 아니기때문에 아무 숫자나  넣어도 된다
...
model.fit(x1_train, [y1_train,y2_train], epochs=100, batch_size=32)
