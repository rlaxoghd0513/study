...
x_train = np.load(path_save_train + 'x_train_o.npy')
y_train = np.load(path_save_train + 'y_train_o.npy')
...
# model.add(Conv2D(64, (3,3), padding = 'same',activation = 'relu', kernal_regularizer = L1(0.01)))
# model.add(BatchNormalization())
# model.add(MaxPooling2D())
# model.add(Conv2D(32, 3, activation = 'relu', padding = 'same', kernal_regularizer = L1(0.01)))
# model.add(BatchNormalization())
# model.add(MaxPooling2D())
# model.add(Conv2D(64,3, activation = 'relu', padding = 'same', kernal_regularizer = L2(0.01)))
# model.add(BatchNormalization())
# model.add(MaxPooling2D())
# model.add(Flatten())
...
model.fit(x_train, y_train, epochs = 1000, batch_size = 8, validation_split= 0.1, callbacks = [es])
