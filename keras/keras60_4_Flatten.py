...
model = Sequential()
model.add(Embedding(27,32, input_length = 5)) #flatten붙이려면 input_length 명시해줘야한다
# model.add(Embedding(27,32,5)) 에러뜸  
# model.add(Embedding(input_dim = 31, output_dim = 32)) flatten바로 못붙임
model.add(Flatten())
model.add(Dense(32))
model.add(Dense(64))
model.add(Dense(32))
...
model.fit(pad_x, labels, epochs = 1000, batch_size = 32 , callbacks=[es])
